...
from tunip.corpus_utils_v2 import CorpusToken, CorpusTokenOnly, CorpusRecord, CorpusSeqLabel, CorpusInput, merge_surface
...
LOGGER = init_logging_handler(name="nugget", level=logging.INFO)
...
class NuggetFilter:
    ...
    @classmethod
    def return_nugget(cls, nugget, nugget_cls) -> CorpusToken:
        return nugget
        # return nugget_cls(**{key: nugget[i] for i, key in enumerate(nugget_cls.__fields__.keys())})

    @classmethod
    def return_nugget_b_e_lex(cls, nugget, nugget_cls) -> CorpusTokenOnly:
        # nugget: CorpusToken
...
class Nugget:
    ...
    def __init__(self, method="POST", tagger_type="seunjeon", use_inflect=False, split_sentence=False):
    ...
    def record_v2(self, texts):
        ...
        try:
            result = self._record(texts)
        except TimeoutError:
            LOGGER.error(f'[NUGGET]: TimeOutError for {texts[0][:self.chunk_length]} ...')
            time.sleep(200)
            result = None
            result = self._record(texts)
        except requests.exceptions.ConnectionError:
            LOGGER.error(f'[NUGGET]: ConnectionError for {texts[0][:self.chunk_length]} ...')
            time.sleep(200)
            result = None
            result = self._record(texts)
        finally:
    ...
    def _record(self, texts):
        ...
        empty_indexes = [i for i, t in enumerate(texts) if t == '' or t is None]
        valid_indexes = [i for i, t in enumerate(texts) if t != '']
        valid_texts = [t for i, t in enumerate(texts) if i in valid_indexes]

        res = self.call_fn(valid_texts)
        ...
        if res.status_code == 200:
            ...
            for i, txt in enumerate(texts):
                ...
                if i in empty_indexes:
                    yield CorpusRecord(text="", tokens=tokens, labels=labels)
                else:
                    res_sent = res_json["sentences"][valid_count]
                    valid_count += 1
                    for res_token in res_sent["tokens"]:
                        tokens.append(
    ...
    def get_token_surfaces(self, texts: list) -> list:
        token_records = self.record_v2(texts)
        token_surfaces = [merge_surface(record.tokens, split_by_token=True) if record.tokens else None for record in token_records]

        return token_surfaces

    def get_token_surfaces_by_chunk(self, texts: list, chunk_size: int):
        processed_texts = [re.sub("\n+", " ", text) if text else "" for text in texts]

        tokenized_texts = []
        for i in range(0, len(processed_texts), chunk_size):
            tokens = self.get_token_surfaces(processed_texts[i:min(i+chunk_size, len(processed_texts))])
            if len(tokens) == chunk_size:
                tokenized_texts.extend(tokens)
            else:
                raise ValueError(f"Texts tokenized wrong. \ntexts: {texts} \ntokens: {tokens}")
    ...
    def post(self, texts):
        body = {
            ...
            "text": "\n".join(texts),
            "splitSentence": self.split_sentence,
    ...
    def filter_v2(self, nuggets, white_tags=[], result_format=NuggetFilterResultFormat.ALL):
        ...
        nugget_tokens = []
        for tokens in preprocess_tokens_v2(nuggets, white_tags=white_tags):
            nugget_tokens.append([return_nugget_func(n, nugget_cls) for n in tokens])
