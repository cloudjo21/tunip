...
def test_it_builds_feature_schema(model_input_feature_schema: dict):
    ...
    ModelInputFeatureSchema = create_model("tunip.ModelInputFeatureSchema", **dict([(k, (numpy_dtype_to_python(np.dtype(v)), '')) for k, v in feature_types.items()]))
...
def test_it_should_be_met_that_dictionary_feature_schema_contains_feature_schema(model_input_feature_schema: dict):
    ...
    ModelInputFeatureSchema2 = create_model("tunip.ModelInputFeatureSchema2", **dict([(k, (numpy_dtype_to_python(np.dtype(v)), '')) for k, v in feature_types.items()]))
    ms2 = ModelInputFeatureSchema2(A=8, B=0.5, C=9.1)

    assert set(ms1.model_dump()).issubset(set(ms2.model_dump())) is True

    assert list(ModelInputFeatureSchema2.model_json_schema().get("properties").keys()) == ["A", "B", "C"]

    # check implicit includeness
    assert ModelInputFeatureSchema.model_validate({"A": 1, "B": 2.2, "C": "..."})

    # check strict includeness
    with pytest.raises(ValidationError):
        ModelInputFeatureSchema.model_validate_json({"A": 1, "B": 2.2, "C": "..."})
