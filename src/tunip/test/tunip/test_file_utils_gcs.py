...
class FileUtilsGcsTest(unittest.TestCase):
    ...
    def test_gcs_handler_mkdirs(self):
        try:
            self.gcs_handler.mkdirs('/user/ed/test/mydir0')
        except Exception as e:
            self.fail(f'test_gcs_handler_mkdirs is failed: {str(e)}')
        
        with self.assertRaises(Exception):
            file_path = self.gcs_handler.build('/user/ed/test/mydir0')
            self.pa_fs.create_dir(file_path)

        # assert mkdirs existhing directory
        try:
            self.gcs_handler.mkdirs('/user/ed/test/mydir0')
        except Exception as e:
            self.fail(f'test_gcs_handler_mkdirs is failed: {str(e)}')
